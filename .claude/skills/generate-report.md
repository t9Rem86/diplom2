...
## Предварительная проверка — есть ли уже параметры?

Перед построением вызови `mcp__finmodel__list_projects`.

**Если проект в списке есть** — покажи ключевые параметры через `mcp__finmodel__get_project_params`
и спроси: «Строить по этим параметрам или изменить?»
- «Строить» / «да» → сразу переходи к «Шаг 1» ниже
- «Изменить» → запусти `/new-project` для обновления параметров, затем вернись сюда

**Если проекта нет** → сначала `/new-project`, затем вернись сюда.
